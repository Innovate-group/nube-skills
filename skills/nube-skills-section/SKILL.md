...
5. **UI-kit del proyecto (SIEMPRE):** antes de generar cualquier componente o section, localizá el ui-kit del rediseño (el archivo/página de Figma con colores, tipografías y componentes base). Buscá su link en el `CLAUDE.md` del proyecto o en tu memoria persistente; si no lo encontrás, **preguntale al dev dónde está y guardalo para no volver a preguntarlo** — preferentemente en el `CLAUDE.md` del proyecto (lo comparte todo el equipo), o en tu memoria si el proyecto no tiene `CLAUDE.md`. Si el proyecto no tiene ui-kit en Figma, el sistema visual son los tokens del tema (`layouts/resources/style-tokens.tpl` + `config/settings_schema.json`).
