...
| `nube-skills-figma-section` | skill | ✅ disponible | De un nodo de Figma (desktop + mobile) a código del tema: triage (configurar / re-estilizar / extender / custom), settings-first, `.tpl` + `{% schema %}` + traducciones + registro en el JSON template. Sin boceto, dibuja primero un mockup con la skill `design` y recién ahí codea. |
